...
def expected_vendor_attack_cost(
    ...
    freq_modifier = (
        (1 - vendor_hardening * 0.40)   # very strong control
        * (1 - iam_strength * 0.25)
        * (1 - api_logging * 0.15)
    )
    ...
    exfil_success = (1 - dlp_coverage * 0.50)  # DLP heavily reduces PHI loss
...
def render_vendor_compromise_tab():
    ...
    # ---------------- Sidebar Controls (Contextual) -------------------
    with st.sidebar:
        st.markdown("### ⚙️ Vendor Scenario Controls")

        vendor_hardening = st.slider(
            "Vendor Access Hardening (%)",
            0.0, 1.0, default_vendor_hardening, step=0.05, key="sidebar_vendor_hardening"
        )

        iam_strength = st.slider(
            "Cloud IAM Restriction Strength (%)",
            0.0, 1.0, default_iam_strength, step=0.05, key="sidebar_iam_strength"
        )

        api_logging = st.slider(
            "API / Cloud Logging Coverage (%)",
            0.0, 1.0, default_api_logging, step=0.05, key="sidebar_api_logging"
        )

        dlp_coverage = st.slider(
            "DLP Coverage (%)",
            0.0, 1.0, default_dlp_coverage, step=0.05, key="sidebar_dlp"
        )

        st.markdown("---")

        mttd = st.number_input(
            "MTTD (hours)", 0.0, 2000.0, default_mttd, step=12.0, key="sidebar_mttd_vendor"
        )
        mttr = st.number_input(
            "MTTR (hours)", 0.0, 500.0, default_mttr, step=4.0, key="sidebar_mttr_vendor"
        )

        downtime_cost_per_hour = st.number_input(
            "Downtime Cost per Hour (USD)",
            0.0, 500000.0, default_downtime_cost, step=5000.0, key="sidebar_downtime_vendor"
        )

        recovery_cost = st.number_input(
            "Recovery Fixed Cost (USD)",
            0.0, 20000000.0, default_recovery_cost, step=25000.0, key="sidebar_recovery_vendor"
        )

        phi_records = st.number_input(
            "PHI Records At Risk",
            0, 500000, int(default_phi_records), key="sidebar_phi_records"
        )

        phi_cost_per_record = st.number_input(
            "Cost per PHI Record (USD)",
            0, 1000, int(default_phi_cost_per_record), key="sidebar_phi_cost"
        )
    ...
    for d in dlp_values:
        c, *_ = expected_vendor_attack_cost(
            ...
            base_vendor_freq, mttd, mttr,
            downtime_cost_per_hour, recovery_cost,
            phi_records, phi_cost_per_record
        )
        dlp_costs.append(c)

    fig = px.line(
        x=dlp_values, y=dlp_costs,
    ...
    # ---------------- Heatmap (IAM vs DLP) ----------------
    st.markdown("### 🌡️ Heatmap: IAM Strength vs DLP Coverage")
